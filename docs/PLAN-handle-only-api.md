...
1. **Fetch / ensure (async, path-based).** The asset layer makes a file local
   ...
   sk_handle_t task = sk_asset_ensure_async(path, NULL);
   sk_asset_add_task(task, on_ready, on_failed, ctx);
   ```
...
2. **Create (sync, path- or handle-based).** Inside the ready callback, build the
   ...
   static void on_model_ready(const char *path, void *user) {
       ctx->model = sk_model_create(path);       /* sync; reads local file */
   }
   ```
...
sk_handle_t sk_font_create(const char *path, int size);
...
sk_handle_t sk_model_create(sk_handle_t mesh);   /* was sk_model_create_from_mesh */
sk_handle_t sk_sound_create(sk_handle_t audio);  /* was sk_sound_create(path)     */
sk_handle_t sk_music_create(sk_handle_t audio);  /* was sk_music_create(path)     */
sk_handle_t sk_text2d_create(sk_handle_t font, float size);
```

### Asset (fetch/ensure) — rework sk_asset to librl's shape
```c
typedef void (*sk_asset_callback_fn)(const char *path, void *user_data);

int         sk_asset_set_host(const char *host);
const char *sk_asset_get_host(void);

int         sk_asset_ensure(const char *path, const char *src);        /* sync */
sk_handle_t sk_asset_ensure_async(const char *path, const char *src);  /* → ASSET_TASK */
sk_asset_add_task_result_t sk_asset_add_task(sk_handle_t task,
                                             ...
                                             void *user_data);
void        sk_asset_tick(void); /* pump the queue each frame (runtime-driven) */
/* + poll/finish/get_task_path/free_task as in librl */
```

`SK_HANDLE_KIND_ASSET_TASK` already exists for the task handle.

### Removed (public)
- **Every `sk_*_create_from_memory(...)`** — texture, mesh, model, sound, music
  (the only pointer-taking functions). Decode-from-bytes stays internal.
- **Object-from-file shortcuts** — `sk_sprite3d_create_from_file`, and the
  bundled `sk_model_create(path)` (mesh+model in one). Objects come from a
  resource handle; load the resource first.
- **Path-based sound/music create** — `sk_sound_create(path)` /
  `sk_music_create(path)` become handle-based (`…create(audio)`).
- `sk_model_create_from_mesh` → renamed `sk_model_create(mesh)`.
- The current byte-delivering `sk_asset_load_async(path, on_loaded(data,size,…))`
  → replaced by the ensure API above (callbacks deliver a path).

### Not doing (corrections to the earlier draft)
- **No generic `sk_asset_load(path) → handle`.** Creation is typed and sync; the
  asset layer ensures files, it doesn't create resources.
- **No generic `sk_destroy(handle)`.** librl keeps typed `sk_*_destroy`; we will
  too. (The 6-bit kind field *could* support a generic destroy later, but it's
  out of scope and not the established model.)

## New dependency: `sk_fs`

Ensure needs a local-file layer (librl's `rl_fs`): a root dir on desktop, idbfs
on web, with fetch-from-host on miss. libsk has **no fs module yet**, so this is
net-new and is the substantive part of the work — `sk_asset` ensure sits on top
of it.

## Phasing

**Phase 1 — kill pointers in user code (desktop-first).**
- Delete public `sk_*_create_from_memory`; keep internal decoders.
- Add object-from-path creators where missing (`sk_sprite3d_create_from_file`).
- Replace `sk_asset`'s byte callback with the path-based ensure API. On desktop,
  "ensure" degenerates to "file exists on disk → fire callback"; the sync
  `sk_*_create(path)` then reads it. (No host fetch yet.)
- Port `examples/*` to ensure-async → `_create(path)`; build clean; `make check`.

**Phase 2 — real ensure (web parity).**
- Add `sk_fs` (root dir + idbfs) and host fetch (sokol_fetch) so `ensure`
  fetches missing files into the local store on web. Group-ensure variants.

**Phase 3 — embedded data (optional).**
- `sk_fs_mount_memory(vpath, data, size)` for compiled-in blobs, so they load via
  `sk_*_create("mem://…")` with no per-asset pointer in gameplay code.
...
- **`sk_audio` is public.** Objects come from resource handles, so a sound needs an
  audio handle — `sk_audio_create(path)` joins the public resource creators.

## Open questions
1. **Scope now:** Phase 1 only (desktop; removes the pointers and reshapes the
   API) and leave `sk_fs`/web fetch for a follow-up, or build Phase 1+2 together?
2. **Music vs Sound:** both are now objects over an Audio handle, differing only
   by default loop. Keep both public surfaces, or collapse to `sk_sound_create`
   + a loop setter and make `sk_music_*` go away? (Separate from this plan, but
   adjacent.)
