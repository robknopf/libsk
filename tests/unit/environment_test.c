...
static vec3_t random_dir(unsigned *state)
{
    for (;;) {
        ...
        if (wgri_v3_dot(v, v) > 0.01f && wgri_v3_dot(v, v) <= 1.0f) return wgri_v3_norm(v);
    }
}

/* an equirect image filled from a function of direction */
static wgri_env_image_t make_image(int width, int height, vec3_t (*fn)(vec3_t))
{
    wgri_env_image_t image = {.width = width, .height = height};
    image.rgb = (float *)malloc((size_t)width * height * 3 * sizeof(float));
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            const vec3_t c = fn(wgri_environment_equirect_dir(((float)x + 0.5f) / width, ((float)y + 0.5f) / height));
...
void test_environment_mapping(void)
{
    ...
    CHECK_VEC3_NEAR(wgri_environment_equirect_dir(0.5f, 0.5f), 0, 0, -1, EPS);
    CHECK_VEC3_NEAR(wgri_environment_equirect_dir(0.75f, 0.5f), 1, 0, 0, EPS);
    CHECK_NEAR(wgri_environment_equirect_dir(0.3f, 0.0f).y, 1, EPS);

    for (int i = 0; i < 200; i++) {
        const vec3_t d = random_dir(&state);
        float u, v, s, t;
        int face;
        wgri_environment_dir_equirect(d, &u, &v);
        CHECK_VEC3_NEAR(wgri_environment_equirect_dir(u, v), d.x, d.y, d.z, EPS);
        wgri_environment_dir_cube(d, &face, &s, &t);
        CHECK(face >= 0 && face < 6 && s >= 0 && s <= 1 && t >= 0 && t <= 1);
        CHECK_VEC3_NEAR(wgri_environment_cube_dir(face, s, t), d.x, d.y, d.z, EPS);
    }
    /* face order +X -X +Y -Y +Z -Z (sokol/OpenGL) */
    CHECK_VEC3_NEAR(wgri_environment_cube_dir(0, 0.5f, 0.5f), 1, 0, 0, EPS);
    CHECK_VEC3_NEAR(wgri_environment_cube_dir(3, 0.5f, 0.5f), 0, -1, 0, EPS);
    CHECK_VEC3_NEAR(wgri_environment_cube_dir(5, 0.5f, 0.5f), 0, 0, -1, EPS);
    /* +X face: s runs toward -z, t toward -y (OpenGL cube map orientation) */
    CHECK(wgri_environment_cube_dir(0, 1.0f, 0.5f).z < -0.5f);
    CHECK(wgri_environment_cube_dir(0, 0.5f, 1.0f).y < -0.5f);

    /* a cubemap made from an image samples like the image */
    wgri_env_image_t image = make_image(128, 64, smooth);
    wgri_env_cube_t cube;
    CHECK(wgri_environment_cube_from_equirect(&image, 32, &cube));
    CHECK(cube.mip_count == 6);
    for (int i = 0; i < 100; i++) {
        const vec3_t d = random_dir(&state);
        const vec3_t a = wgri_environment_sample_equirect(&image, d);
        const vec3_t b = wgri_environment_sample_cube(&cube, d, 0.0f);
...
void test_environment_irradiance(void)
{
    ...
    wgri_env_image_t image = make_image(128, 64, constant_two);
    wgri_environment_project_sh(&image, &sh);
    for (int i = 0; i < 20; i++) {
        const vec3_t e = wgri_environment_eval_sh(&sh, random_dir(&state));
    ...
    wgri_environment_project_sh(&image, &sh);
    CHECK_NEAR(wgri_environment_eval_sh(&sh, (vec3_t){0, 1, 0}).x, 2.0f / 3.0f, 0.03f);
    CHECK_NEAR(wgri_environment_eval_sh(&sh, (vec3_t){0, -1, 0}).x, 0.0f, 0.05f);
    /* facing the horizon: E = integral of y * x over the quarter sphere x, y > 0 = 2/3, so E/pi = 2/(3pi) */
    CHECK_NEAR(wgri_environment_eval_sh(&sh, (vec3_t){1, 0, 0}).x, 2.0f / (3.0f * 3.14159265f), 0.02f);
    free(image.rgb);
}

void test_environment_prefilter(void)
{
    wgri_env_cube_t source, filtered;

    /* a constant environment stays constant at every roughness */
    wgri_env_image_t image = make_image(128, 64, constant_two);
    CHECK(wgri_environment_cube_from_equirect(&image, 32, &source));
    CHECK(wgri_environment_prefilter(&source, 16, 4, 32, &filtered));
    ...
    wgri_environment_cube_free(&source);
    wgri_environment_cube_free(&filtered);
    free(image.rgb);

    /* blurring keeps the average: each mip's mean matches the source's */
    image = make_image(128, 64, sky);
    CHECK(wgri_environment_cube_from_equirect(&image, 32, &source));
    CHECK(wgri_environment_prefilter(&source, 16, 4, 64, &filtered));
    ...
    const vec3_t up0 = wgri_environment_sample_cube(&filtered, (vec3_t){0, 1, 0}, 0);
    const vec3_t up3 = wgri_environment_sample_cube(&filtered, (vec3_t){0, 1, 0}, 3);
    const vec3_t down0 = wgri_environment_sample_cube(&filtered, (vec3_t){0, -1, 0}, 0);
    const vec3_t down3 = wgri_environment_sample_cube(&filtered, (vec3_t){0, -1, 0}, 3);
...
void test_environment_brdf_lut(void)
{
    enum { N = 32 };
    float lut[N * N * 2];
    wgri_environment_brdf_lut(N, 128, lut);
...
{
    enum { N = WGRI_ENVIRONMENT_LUT_SIZE };
...
{
    CHECK(wgri_environment_half_from_float(0.0f) == 0x0000);
    CHECK(wgri_environment_half_from_float(1.0f) == 0x3C00);
    CHECK(wgri_environment_half_from_float(0.5f) == 0x3800);
    CHECK(wgri_environment_half_from_float(-2.0f) == 0xC000);
    CHECK(wgri_environment_half_from_float(65504.0f) == 0x7BFF);
    CHECK(wgri_environment_half_from_float(1e9f) == 0x7BFF); /* clamped to the largest half */
    CHECK(wgri_environment_half_from_float(1e-9f) == 0x0000);
    /* expected values from Python's struct 'e' (IEEE 754 half, round to nearest) */
    CHECK(wgri_environment_half_from_float(0.1f) == 0x2E66);
    CHECK(wgri_environment_half_from_float(1e-4f) == 0x068E);
    CHECK(wgri_environment_half_from_float(3e-5f) == 0x01F7); /* subnormal */
    CHECK(wgri_environment_half_from_float(1e-5f) == 0x00A8); /* subnormal */
    CHECK(wgri_environment_half_from_float(65519.0f) == 0x7BFF); /* rounds up past the largest finite: clamped */
...
void test_environment_api(void)
{
    ...
    if (!sg_query_pixelformat(SG_PIXELFORMAT_RGBA16F).filter) {
        ...
        wgri_environment_get_binding(0, &binding);
