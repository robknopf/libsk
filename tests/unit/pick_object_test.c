...
void test_pick_object(void)
{
    ...
    sk_handle_t front = sk_shape_create(), back = sk_shape_create();
    sk_shape_set_cube(front, 2, 2, 2);
    sk_shape_set_cube(back, 2, 2, 2);
    sk_shape_set_transform(back, 0, 0, -5, 0, 0, 0, 1, 1, 1);
    ...
    sk_shape_set_transform(front, 50, 0, 0, 0, 0, 0, 1, 1, 1);
    ...
    sk_shape_set_transform(front, 0, 0, 0, 0, 0, 0, 1, 1, 1);
    ...
    CHECK(r.hit && r.handle == front);
    CHECK(sk_shape_set_pickable(front, false));
    r = sk_pick_object(front, camera, 0.5f, 0.5f);
    CHECK(!r.hit);
    r = sk_scene_pick(scene, camera, 0.5f, 0.5f);
    CHECK(r.hit && r.handle == back);
    CHECK_NEAR(r.distance, 13.99f, EPS);
    sk_shape_set_visible(back, false); /* hidden objects aren't hit either */
    r = sk_scene_pick(scene, camera, 0.5f, 0.5f);
    CHECK(!r.hit);
...
void test_shape_3d(void)
{
    ...
    CHECK(sk_shape_set_rectangle(rect, 2, 1));
    ...
    sk_shape_set_transform(rect, 0.9f, 0, 0, 0, 0, 0, 1, 1, 1); /* still covers the center (half width 1) */
    CHECK(sk_pick_object(rect, camera, 0.5f, 0.5f).hit);
    sk_shape_set_transform(rect, 1.1f, 0, 0, 0, 0, 0, 1, 1, 1); /* now just misses */
    CHECK(!sk_pick_object(rect, camera, 0.5f, 0.5f).hit);
    sk_shape_set_transform(rect, 0, 0, 0, 0, 1.5707963f, 0, 1, 1, 1); /* edge-on */
    CHECK(!sk_pick_object(rect, camera, 0.5f, 0.5f).hit);

    /* circles are picked anywhere inside the outline */
    sk_handle_t circle = sk_shape_create();
    CHECK(sk_shape_set_circle(circle, 1));
    sk_shape_set_transform(circle, 0.7f, 0, 0, 0, 0, 0, 1, 1, 1);
    CHECK(sk_pick_object(circle, camera, 0.5f, 0.5f).hit);
    sk_shape_set_transform(circle, 1.2f, 0, 0, 0, 0, 0, 1, 1, 1);
    CHECK(!sk_pick_object(circle, camera, 0.5f, 0.5f).hit);

    /* lines have no area */
    sk_handle_t line = sk_shape_create();
    CHECK(sk_shape_set_line(line, -1, 0, 0, 1, 0, 0));
    CHECK(!sk_pick_object(line, camera, 0.5f, 0.5f).hit);

    /* strips: built point by point, emptied by set_line_strip */
    sk_handle_t strip = sk_shape_create();
    CHECK(!sk_shape_add_point(strip, 0, 0, 0)); /* not a strip yet */
    CHECK(sk_shape_set_line_strip(strip));
    for (int i = 0; i < 100; i++) {
        CHECK(sk_shape_add_point(strip, (float)i, (float)(i % 3), 0));
    }
    CHECK(sk_shape_get_point_count(strip) == 100);
    vec3_t lmin, lmax;
    sk_mat4_t model;
    ...
    CHECK(sk_shape_set_line_strip(strip));
    CHECK(sk_shape_get_point_count(strip) == 0);
    CHECK(!sk_drawable_bounds(strip, &lmin, &lmax, &model)); /* empty: nothing to pick */
    CHECK(sk_shape_set_cube(strip, 1, 1, 1));
    CHECK(sk_shape_get_point_count(strip) == 0);
