...
{
    wgri_gamepad_frame_done();
    wgri_gamepad_end_tick();
    wgri_gamepad_begin_frame();
}
...
void test_gamepad_buttons(void)
{
    ...
    wgri_gamepad_set_test_pad(1, true, "Test Pad", buttons, axes);
    frame();
    CHECK(!wgr_input_is_gamepad_connected(0));
    ...
    wgri_gamepad_frame_done(); /* the frame is done; no tick ran */
    ...
    wgri_gamepad_set_test_pad(1, false, NULL, NULL, NULL);
    frame();
    CHECK(!wgr_input_is_gamepad_connected(1));
...
void test_gamepad_axes(void)
{
    ...
    wgri_gamepad_set_test_pad(0, true, "Test Pad", buttons, axes);
    frame();
