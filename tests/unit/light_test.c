...
static wgr_scene_light_t directional(float brightness)
{
    return (wgr_scene_light_t){.type = WGR_LIGHT_DIRECTIONAL, .radiance = {brightness, brightness, brightness},
                              .direction = {0, -1, 0}};
}

static wgr_scene_light_t point(float x, float y, float z, float brightness, float range)
{
    return (wgr_scene_light_t){.type = WGR_LIGHT_POINT, .radiance = {brightness, brightness, brightness},
...
void test_light_falloff(void)
{
    ...
    CHECK_NEAR(wgr_light_attenuation(2.0f, 0.0f), 0.25f, EPS);
    CHECK_NEAR(wgr_light_attenuation(0.0f, 0.0f), 100.0f, EPS); /* distance clamped at 0.1 */
    /* smooth window: unchanged near the light, exactly 0 at and past the range */
    CHECK_NEAR(wgr_light_attenuation(1.0f, 10.0f), 1.0f * (1.0f - 1e-4f) * (1.0f - 1e-4f), EPS);
    CHECK_NEAR(wgr_light_attenuation(10.0f, 10.0f), 0.0f, EPS);
    CHECK_NEAR(wgr_light_attenuation(12.0f, 10.0f), 0.0f, EPS);
    CHECK(wgr_light_attenuation(9.0f, 10.0f) < wgr_light_attenuation(5.0f, 10.0f));

    /* spot cone: full inside inner, zero outside outer, smooth between */
    const float cos_inner = 0.9f, cos_outer = 0.7f;
    CHECK_NEAR(wgr_light_spot_factor(0.95f, cos_inner, cos_outer), 1.0f, EPS);
    CHECK_NEAR(wgr_light_spot_factor(0.5f, cos_inner, cos_outer), 0.0f, EPS);
    CHECK_NEAR(wgr_light_spot_factor(0.8f, cos_inner, cos_outer), 0.5f, EPS);
    /* inner == outer: hard edge */
    CHECK_NEAR(wgr_light_spot_factor(0.71f, 0.7f, 0.7f), 1.0f, EPS);
    CHECK_NEAR(wgr_light_spot_factor(0.69f, 0.7f, 0.7f), 0.0f, EPS);
}

void test_light_select(void)
{
    static wgr_light_env_t env;
    int picked[WGR_MAX_DRAW_LIGHTS];
    ...
    CHECK(wgr_light_select(&env, bmin, bmax, picked, WGR_MAX_DRAW_LIGHTS) == 4);
    ...
    CHECK(wgr_light_select(&env, bmin, bmax, picked, WGR_MAX_DRAW_LIGHTS) == 1);
    CHECK(picked[0] == 1);
    ...
    CHECK(wgr_light_select(&env, bmin, bmax, picked, WGR_MAX_DRAW_LIGHTS) == 2);
    CHECK(picked[0] == 1 && picked[1] == 0);

    /* a spot aimed away from the model doesn't reach it */
    env.count = 2;
    env.lights[0] = (wgr_scene_light_t){.type = WGR_LIGHT_SPOT, .radiance = {5, 5, 5}, .position = {0, 5, 0},
    ...
    CHECK(wgr_light_select(&env, bmin, bmax, picked, WGR_MAX_DRAW_LIGHTS) == WGR_MAX_DRAW_LIGHTS);
    for (int i = 0; i < WGR_MAX_DRAW_LIGHTS; i++) {
        CHECK(picked[i] == 11 - i);
    }
}
...
void test_light_api(void)
{
    ...
    CHECK(wgr_light_get_scene_light(light, &data));
