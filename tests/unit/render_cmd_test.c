...
void test_render_command_merging(void)
{
    ...
    wgr_render_begin();
    const int start = wgr_render_command_count();
    ...
    wgr_render_submit_models(0, 2);
    const int after_first = wgr_render_command_count();
    ...
    wgr_render_submit_models(2, 3);
    CHECK(wgr_render_command_count() == after_first);
    wgr_render_submit_models(5, 1);
    CHECK(wgr_render_command_count() == after_first);
    ...
    wgr_render_submit_models(20, 1);
    CHECK(wgr_render_command_count() == after_first + 1);

    /* nothing to submit changes nothing */
    wgr_render_submit_models(21, 0);
    wgr_render_submit_models(21, -3);
    CHECK(wgr_render_command_count() == after_first + 1);
    ...
    const int after_2d = wgr_render_command_count();
    wgr_render_submit_models(21, 1);
    CHECK(wgr_render_command_count() == after_2d + 2);
    wgr_render_end();

    /* a sprite batch is open only while its command is the last one recorded */
    wgr_render_begin();
    CHECK(!wgr_render_sprites_open(3));
    CHECK(wgr_render_submit_sprites(3));
    CHECK(wgr_render_sprites_open(3));
    CHECK(!wgr_render_sprites_open(4)); /* a different batch isn't open */
    wgr_shape2d_draw_rectangle(1, 1, 10, 10, WGR_COLOR_WHITE);
    CHECK(!wgr_render_sprites_open(3)); /* something was drawn over it */
    CHECK(wgr_render_submit_sprites(3));
    CHECK(wgr_render_sprites_open(3));
    wgr_render_submit_models(0, 1);
    CHECK(!wgr_render_sprites_open(3));
    wgr_render_end();
    ...
    drawn = 0;
    wgr_render_begin();
    wgr_render_submit_callback(count_draw, 1);
    wgr_render_submit_callback(count_draw, 10);
    wgr_render_submit_callback(NULL, 100); /* nothing to run */
    wgr_render_end();
    CHECK(drawn == 11);
...
void test_render_command_passes(void)
{
    ...
    const int in_target = wgr_render_command_count();
    wgr_render_submit_models(2, 2); /* would carry on, but it's another pass */
    CHECK(wgr_render_command_count() == in_target + 1);
    wgr_render_submit_models(4, 1); /* and inside the pass they do merge */
    CHECK(wgr_render_command_count() == in_target + 1);
    ...
    const int back = wgr_render_command_count();
    wgr_render_submit_models(5, 1);
    CHECK(wgr_render_command_count() == back + 2);
    wgr_render_end();
