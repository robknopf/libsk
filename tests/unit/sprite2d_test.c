...
static wgr_sprite2d_placement_t placement(float rotation, float scale_x, float scale_y, float pivot_x, float pivot_y)
{
    /* 40 x 20 sprite with its pivot at (100, 50) */
    return (wgr_sprite2d_placement_t){.x = 100, .y = 50, .width = 40, .height = 20, .scale_x = scale_x,
...
void test_sprite2d_corners(void)
{
    ...
    wgr_sprite2d_placement_t p = placement(0, 1, 1, 0.5f, 0.5f);
    wgr_sprite2d_corners(&p, c);
...
void test_sprite2d_screen_to_unit(void)
{
    ...
    wgr_sprite2d_placement_t p = placement(0, 1, 1, 0.5f, 0.5f);
    CHECK(wgr_sprite2d_screen_to_unit(&p, 100, 50, &u, &v)); /* center */
    CHECK_NEAR(u, 0.5f, EPS); CHECK_NEAR(v, 0.5f, EPS);
    CHECK(wgr_sprite2d_screen_to_unit(&p, 81, 41, &u, &v));  /* near top-left */
    CHECK_NEAR(u, 0.025f, EPS); CHECK_NEAR(v, 0.05f, EPS);
    CHECK(!wgr_sprite2d_screen_to_unit(&p, 79, 50, &u, &v)); /* just left of it */
    CHECK(!wgr_sprite2d_screen_to_unit(&p, 100, 61, &u, &v)); /* just below */

    /* scaled 2x: the same point is closer to the center in unit space */
    p = placement(0, 2, 2, 0.5f, 0.5f);
    CHECK(wgr_sprite2d_screen_to_unit(&p, 79, 50, &u, &v));
