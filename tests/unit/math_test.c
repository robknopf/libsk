...
{
    sk_mat4_t m = sk_mat4_trs((vec3_t){1, 2, 3}, (vec3_t){0.3f, 0.5f, -0.2f}, (vec3_t){2, 0.5f, 1.5f});
    sk_mat4_t product = sk_mat4_mul(m, sk_mat4_inverse(m));
    sk_mat4_t identity = sk_mat4_identity();
    for (int i = 0; i < 16; i++) {
        CHECK_NEAR(product.m[i], identity.m[i], EPS);
    }

    vec3_t p = {-4, 7, 0.5f};
    vec3_t round_trip = sk_mat4_mul_point(sk_mat4_inverse(m), sk_mat4_mul_point(m, p));
    CHECK_VEC3_NEAR(round_trip, p.x, p.y, p.z, EPS);

    /* documented: a singular matrix inverts to identity */
    sk_mat4_t singular = sk_mat4_inverse(sk_mat4_scale(0, 1, 1));
    for (int i = 0; i < 16; i++) {
...
void test_math_trs(void)
{
    ...
    vec3_t p = sk_mat4_mul_point(sk_mat4_trs(origin, (vec3_t){0, HALF_PI, 0}, unit), (vec3_t){1, 0, 0});
    CHECK_VEC3_NEAR(p, 0, 0, -1, EPS);
    p = sk_mat4_mul_point(sk_mat4_trs(origin, (vec3_t){0, 0, HALF_PI}, unit), (vec3_t){1, 0, 0});
    CHECK_VEC3_NEAR(p, 0, 1, 0, EPS);

    /* scale, then rotate, then translate */
    p = sk_mat4_mul_point(sk_mat4_trs((vec3_t){10, 0, 0}, (vec3_t){0, 0, HALF_PI}, (vec3_t){2, 1, 1}),
                          (vec3_t){1, 0, 0});
