...
{
    char out[256];
    const bool ok = sk_asset_join_relative(base, uri, out, sizeof(out));
...
void test_asset_join_relative(void)
{
    ...
    char small[8];
    CHECK(!sk_asset_join_relative("models/box.gltf", "texture.png", small, sizeof(small))); /* doesn't fit */

    CHECK(sk_asset_is_relative_uri("box.bin"));
    CHECK(sk_asset_is_relative_uri("../textures/a.png"));
    CHECK(sk_asset_is_relative_uri("dir/with:colon.png")); /* a colon after a slash isn't a scheme */
    CHECK(!sk_asset_is_relative_uri("data:application/octet-stream;base64,AAAA"));
    CHECK(!sk_asset_is_relative_uri("https://example.com/box.bin"));
    CHECK(!sk_asset_is_relative_uri("/abs/box.bin"));
    CHECK(!sk_asset_is_relative_uri(""));
    CHECK(!sk_asset_is_relative_uri(NULL));
}
