...
static const wgri_camera3d_t CAM = {
    .position = {0, 0, 10},
    .target = {0, 0, 0},
    .up = {0, 1, 0},
    .fov = 0.785398163f,
...
{
    CHECK_NEAR(wgri_scene_view_depth(&CAM, (vec3_t){0, 0, 0}), 10, EPS);
    CHECK_NEAR(wgri_scene_view_depth(&CAM, (vec3_t){0, 0, -5}), 15, EPS);
    CHECK_NEAR(wgri_scene_view_depth(&CAM, (vec3_t){3, 4, 10}), 0, EPS); /* sideways offset doesn't count */
    CHECK_NEAR(wgri_scene_view_depth(&CAM, (vec3_t){0, 0, 12}), -2, EPS); /* behind the camera */
}
...
void test_scene_sort_transparent(void)
{
    ...
    wgri_scene_sort_transparent(items, 5);
    for (int i = 0; i < 5; i++) {
        CHECK(items[i].handle == expected[i]);
    }

    /* empty and single-item lists are left alone */
    wgri_scene_sort_transparent(NULL, 0);
    wgri_transparent_item_t one = {.handle = 7, .depth = 3};
    wgri_scene_sort_transparent(&one, 1);
    CHECK(one.handle == 7);
}
...
{
    enum { N = 3000 };
    static wgri_transparent_item_t items[N];
    unsigned seed = 3u;
    bool ordered = true;

    for (int i = 0; i < N; i++) {
        seed = seed * 1664525u + 1013904223u;
        items[i] = (wgri_transparent_item_t){.handle = (wgr_handle_t)(i + 1), .order = i,
                                           .depth = (float)((int)(seed >> 16) % 400 - 100) * 0.25f};
    }
    wgri_scene_sort_transparent(items, N);
    for (int i = 1; i < N; i++) {
