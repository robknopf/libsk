/* Optional subsystems register themselves when linked (internal/sk_module.h): the test
...
void test_module_registry(void)
{
    ...
    sk_module_register(&probe);
    for (const sk_module_t *m = sk_module_list(); m != NULL; m = m->next, position++) {
