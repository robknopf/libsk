 ...
 * convex shapes). */
static void check_shape(const wgri_mesh_shape_t *s, void (*center)(const float *p, float *c))
{
...
static void bounds(const wgri_mesh_shape_t *s, float min[3], float max[3])
{
...
void test_mesh_shapes(void)
{
    ...
    CHECK(wgri_mesh_shape_plane(4, 2, 0, &s));
    ...
    wgri_mesh_shape_free(&s);
    CHECK(wgri_mesh_shape_plane(4, 2, 3, &s));
    CHECK(s.vertex_count == 25 && s.index_count == 4 * 4 * 6);
    wgri_mesh_shape_free(&s);

    CHECK(wgri_mesh_shape_cube(2, 4, 6, &s));
    ...
    CHECK(wgri_mesh_shape_sphere(1.5f, 8, 16, &s));
    ...
    CHECK(wgri_mesh_shape_cylinder(1, 2, 12, &s));
    ...
    CHECK(wgri_mesh_shape_cone(1, 2, 12, &s));
    ...
    CHECK(wgri_mesh_shape_capsule(0.5f, 2, 8, 12, &s));
    ...
    wgri_mesh_shape_free(&s);
    CHECK(wgri_mesh_shape_capsule(0.5f, 0.2f, 8, 12, &s)); /* shorter than its ends: a sphere */
    bounds(&s, lo, hi);
    CHECK_NEAR(hi[1], 0.5f, EPS);
    wgri_mesh_shape_free(&s);

    CHECK(wgri_mesh_shape_torus(2, 0.5f, 24, 12, &s));
    ...
    CHECK(wgri_mesh_shape_sphere(1, 0, 0, &s));
    CHECK(s.vertex_count == (WGRI_MESH_MIN_RINGS + 1) * (WGRI_MESH_MIN_SEGMENTS + 1));
    wgri_mesh_shape_free(&s);
    CHECK(!wgri_mesh_shape_plane(0, 1, 0, &s) && s.positions == NULL);
    CHECK(!wgri_mesh_shape_cube(1, -1, 1, &s));
    CHECK(!wgri_mesh_shape_sphere(NAN, 8, 8, &s));
    CHECK(!wgri_mesh_shape_torus(1, 0, 8, 8, &s));
}
