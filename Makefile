# libwgrender — sokol backend (desktop GL first)
#
...
#   make web        web lib (build/webgl2/libwgrender.a); BACKEND=webgpu, WEB_THREADS=0
...
# GL/X11/ALSA link dependencies (see src/wgr_platform.c). For tests, CI and tools.
HEADLESS ?= 0
...
ifeq ($(WEB),1)
  ...
  CC    := $(MINGW)gcc
  AR    := $(MINGW)ar
  DEFS  := $(if $(filter 1,$(HEADLESS)),-DWGR_HEADLESS -DSOKOL_DUMMY_BACKEND,-DSOKOL_GLCORE)
else ifeq ($(HEADLESS),1)
  DEFS  := -DWGR_HEADLESS -DSOKOL_DUMMY_BACKEND
else
  DEFS  := -DSOKOL_GLCORE
endif
...
print-web-flags-inner:
	@echo "lib:     build/$(WEB_DIR)/libwgrender.a"
...
# `make examples` / `make wasm` build libwgrender first if needed. Command-line vars
# (BACKEND=, WASM_EXAMPLE=) propagate to the sub-make automatically.
examples:
	@$(MAKE) -C examples
run wasm wasm-all websize serve serve-tls spritebench-web shadowbench-web loadbench-web stress-web webcheck webstart smoke:
	@$(MAKE) -C examples $@
...
# carries its own; include them through src/internal/wgr_shaders.h. The binary is
...
# by tools/shaderpack.py into the committed examples/assets/shaders/*.wgrshader.
...
example-shaders:
	@for s in $(EXAMPLE_SHADERS); do \
	    python3 tools/shaderpack.py $$s -o examples/assets/shaders/$$(basename $${s%.glsl}).wgrshader || exit 1; \
	done
