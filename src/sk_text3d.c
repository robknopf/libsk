...
typedef struct {
    ...
    sk_text_align_t align_x, align_y; /* where `position` sits on the block */
    sk_color_t color;
    bool visible;
    bool pickable;
 ...
 * splitting text2d uses (sk_text_split_lines), since text3d draws its own glyphs. */
typedef struct {
...
static bool layout_block(sk_handle_t font, const char *text, float size, float max_width, text3d_block_t *out)
{
    FONScontext *fons = sk_font_context();
...
static vec2_t text_extent(sk_handle_t font, const char *text, float size, float max_width)
{
...
static vec3_t block_center(vec3_t position, vec3_t right, vec3_t up, vec2_t extent, sk_text_align_t align_x,
                           sk_text_align_t align_y)
{
    const float dx = align_x == SK_TEXT_ALIGN_LEFT ? extent.x * 0.5f
                     : align_x == SK_TEXT_ALIGN_RIGHT ? -extent.x * 0.5f
                                                      : 0.0f;
    const float dy = align_y == SK_TEXT_ALIGN_TOP ? -extent.y * 0.5f
                     : align_y == SK_TEXT_ALIGN_BOTTOM ? extent.y * 0.5f
                                                       : 0.0f;
...
static void draw_text(sk_handle_t font, const char *text, float size, float max_width, sk_text_align_t align_x,
                      sk_text_align_t align_y, vec3_t position, vec3_t right, vec3_t up, sk_color_t color)
{
    FONScontext *fons = sk_font_context();
    FONStextIter iter;
    FONSquad quad;
    ...
    for (int i = 0; i < block.count; i++) {
        const float indent = align_x == SK_TEXT_ALIGN_CENTER   ? (block.width - block.widths[i]) * 0.5f
                             : align_x == SK_TEXT_ALIGN_RIGHT ? block.width - block.widths[i]
                                                              : 0.0f;
...
sk_handle_t sk_text3d_create(sk_handle_t font)
{
    ...
    sk_text3ds[index] = (sk_text3d_t){
        ...
        .align_x = SK_TEXT_ALIGN_CENTER,
        .align_y = SK_TEXT_ALIGN_MIDDLE,
...
SK_KEEP
SK_KEEP
bool sk_text3d_set_align(sk_handle_t handle, sk_text_align_t horizontal, sk_text_align_t vertical)
{
    sk_text3d_t *text_ptr = resolve(handle);
    if (text_ptr == NULL) return false;
    if (horizontal > SK_TEXT_ALIGN_RIGHT || vertical < SK_TEXT_ALIGN_TOP || vertical > SK_TEXT_ALIGN_BOTTOM) {
        log_warn("sk_text3d_set_align: horizontal is LEFT/CENTER/RIGHT, vertical TOP/MIDDLE/BOTTOM");
        return false;
    }
...
SK_KEEP
bool sk_text3d_set_max_width(sk_handle_t handle, float width)
{
    sk_text3d_t *text_ptr = resolve(handle);
    if (text_ptr == NULL) return false;
...
void sk_text_draw_3d(sk_handle_t font, const char *text, float x, float y, float z, float size, sk_color_t color)
{
    ...
    draw_text(font, text, size, 0.0f, SK_TEXT_ALIGN_CENTER, SK_TEXT_ALIGN_MIDDLE, (vec3_t){x, y, z}, right,
              up, color);
}
