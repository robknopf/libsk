#ifndef WGR_INTERNAL_MODEL_H
#define WGR_INTERNAL_MODEL_H
...
void wgr_model_flush(void); /* the frame's joint matrices into the joint texture */
...
int wgr_model_get_joint_matrices(wgr_handle_t model, const float **matrices);
...
vec3_t wgr_model_skin_position(const wgr_mat4_t *joints, int joint_count, vec3_t position,
