#ifndef WGR_INTERNAL_SCENE_H
#define WGR_INTERNAL_SCENE_H
...
} wgr_transparent_item_t;

typedef void (*wgr_drawable_draw_opaque_fn)(wgr_handle_t handle);
typedef int (*wgr_drawable_collect_transparent_fn)(wgr_handle_t handle, const wgr_camera3d_t *cam,
                                                  wgr_transparent_item_t *out, int max_items);
typedef void (*wgr_drawable_draw_transparent_fn)(wgr_handle_t handle, int part);
...
typedef void (*wgr_drawable_draw_2d_fn)(wgr_handle_t handle);
typedef bool (*wgr_drawable_pick_2d_fn)(wgr_handle_t handle, float screen_x, float screen_y,
 ...
 * (e.g. not yet loaded). */
typedef bool (*wgr_drawable_bounds_fn)(wgr_handle_t handle,
...
bool wgr_drawable_bounds(wgr_handle_t handle, vec3_t *local_min, vec3_t *local_max,
                        wgr_mat4_t *model);
...
typedef bool (*wgr_drawable_pick_fn)(wgr_handle_t handle, vec3_t origin, vec3_t dir,
                                    wgr_pick_result_t *out);
...
typedef bool (*wgr_drawable_enabled_fn)(wgr_handle_t handle);
 ...
 * and text don't). */
typedef bool (*wgr_drawable_casts_shadow_fn)(wgr_handle_t handle);
...
/* What scenes reach through optional modules (internal/wgr_module.h): set by each
...
typedef struct {
    ...
    bool (*scene_light)(wgr_handle_t light, wgr_scene_light_t *out); /* wgr_light */
    int (*light_env_push)(const wgr_light_env_t *env);
    void (*light_env_set_current)(int index);
    const wgr_light_env_t *(*light_env_get)(int index); /* culling asks it which lights cast */
...
extern wgr_scene_hooks_t wgr_scene_hooks;

#endif // WGR_INTERNAL_SCENE_H
