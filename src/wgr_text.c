 ...
 * set with wgr_text_set_default_font, else the built-in font (an ASCII subset of
 * JetBrains Mono embedded in the library; src/fonts/wgr_default_font.h). A font that
 * isn't loaded (invalid or freed handle) falls back to the built-in font. */

#define WGR_TEXT_DEFAULT_SIZE 16.0f /* pixel size when a size <= 0 is given */
...
static bool use_font(wgr_handle_t font, float size, float scale)
{
    ...
    fonsSetFont(fons, id);
    fonsSetSize(fons, (size > 0.0f ? size : WGR_TEXT_DEFAULT_SIZE) * scale);
...
static void draw_line(const char *start, const char *end, int index, void *user)
{
    ...
    if (ctx->align_x == WGR_TEXT_ALIGN_CENTER) {
        x += (ctx->box_width - span_width(ctx->fons, start, end)) * 0.5f;
    } else if (ctx->align_x == WGR_TEXT_ALIGN_RIGHT) {
