...
static sk_environment_t sk_environments[MAX_ENVIRONMENTS];
static sk_handle_pool_t sk_environment_pool;
static uint16_t sk_environment_free_indices[MAX_ENVIRONMENTS];
static uint16_t sk_environment_generations[MAX_ENVIRONMENTS];
static unsigned char sk_environment_occupied[MAX_ENVIRONMENTS];
...
static sk_handle_t find_by_path(const char *path)
{
    ...
    for (uint16_t i = 1; i < MAX_ENVIRONMENTS; i++) {
        if (sk_environment_occupied[i] && sk_environments[i].has_path && strcmp(sk_environments[i].path, path) == 0) {
...
void sk_environment_init(void)
{
    ...
    memset(sk_environments, 0, sizeof(sk_environments));
    sk_handle_pool_init(&sk_environment_pool, SK_HANDLE_KIND_ENVIRONMENT, MAX_ENVIRONMENTS,
                        sk_environment_free_indices, MAX_ENVIRONMENTS,
                        sk_environment_generations, sk_environment_occupied);
...
void sk_environment_deinit(void)
{
    ...
    memset(&sk_env, 0, sizeof(sk_env));
    sk_handle_pool_reset(&sk_environment_pool);
}
