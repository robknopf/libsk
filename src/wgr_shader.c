...
static wgr_shader_t *wgr_shaders; /* grown by the pool: don't hold a pointer across a create */
static wgr_handle_pool_t wgr_shader_pool;

/* wgr_shader_hooks.fallbacks */
static struct {
...
static wgr_shader_t *resolve(wgr_handle_t handle)
{
    uint16_t index = 0;
    if (!wgr_handle_pool_resolve(&wgr_shader_pool, handle, &index)) {
...
{
    uint16_t index = 0;
    return shader != 0 && wgr_handle_pool_resolve(&wgr_shader_pool, shader, &index) ? &wgr_shaders[index] : NULL;
}
...
typedef struct {
    ...
    int name_count;
    char view_names[SG_MAX_VIEW_BINDSLOTS][WGR_SHADER_NAME_MAX];
    int pair_view[SG_MAX_TEXTURE_SAMPLER_PAIRS];
    int pair_sampler[SG_MAX_TEXTURE_SAMPLER_PAIRS];
    int pair_count;
    bool has_block[WGR_SHADER_BLOCK_COUNT];
    bool found;
} program_desc_t;
...
static bool describe(program_desc_t *p, const char *line)
{
    ...
    if (sscanf(line, "ub %d %63s %d %127s %d %d", &slot, a, &n1, b, &n2, &n3) == 6) {
        if (slot < 0 || slot >= WGR_SHADER_BLOCK_COUNT) return false;
    ...
    if (sscanf(line, "view %d %63s %63s %63s %63s %d", &slot, a, b, d, c, &n1) == 6) {
        ...
        snprintf(p->view_names[slot], WGR_SHADER_NAME_MAX, "%.*s", WGR_SHADER_NAME_MAX - 1, b);
        return true;
    }
...
static wgr_shader_param_type_t param_type(const char *s, bool *ok)
{
    *ok = true;
    if (strcmp(s, "float") == 0) return WGR_SHADER_PARAM_FLOAT;
    if (strcmp(s, "int") == 0) return WGR_SHADER_PARAM_INT;
    if (strcmp(s, "vec2") == 0) return WGR_SHADER_PARAM_VEC2;
    if (strcmp(s, "vec3") == 0) return WGR_SHADER_PARAM_VEC3;
    if (strcmp(s, "vec4") == 0) return WGR_SHADER_PARAM_VEC4;
    *ok = false;
    return WGR_SHADER_PARAM_FLOAT;
}

static int param_size(wgr_shader_param_type_t type)
{
    switch (type) {
        case WGR_SHADER_PARAM_VEC2: return 8;
        case WGR_SHADER_PARAM_VEC3: return 12;
        case WGR_SHADER_PARAM_VEC4: return 16;
        default: return 4;
    }
}
...
static bool wants_program(const wgr_shader_t *shader, int program)
{
    return shader->screen ? program == WGR_SHADER_PROGRAM_SCREEN : program != WGR_SHADER_PROGRAM_SCREEN;
}
...
static bool parse(const unsigned char *bytes, size_t size, const char *slang, wgr_shader_t *out,
                  program_desc_t programs[WGR_SHADER_PROGRAM_COUNT], const char **error)
{
    ...
    while (next_line(&r, line, sizeof(line))) {
        if (strcmp(line, "end") == 0) {
            ...
            bool ok;
            wgr_shader_param_t *param = &out->params[out->param_count];
            if (out->param_count >= WGR_SHADER_MAX_PARAMS || strlen(a) >= WGR_SHADER_NAME_MAX) {
                *error = "too many parameters, or a name too long";
                return false;
            }
            snprintf(param->name, sizeof(param->name), "%s", a);
            param->type = param_type(b, &ok);
            param->block = strcmp(c, "vs") == 0 ? WGR_SHADER_BLOCK_VS_PARAMS : WGR_SHADER_BLOCK_FS_PARAMS;
            param->offset = n;
            if (!ok || n < 0) {
            ...
            if (out->texture_count >= WGR_SHADER_MAX_TEXTURES || strlen(a) >= WGR_SHADER_NAME_MAX) {
                *error = "too many textures, or a name too long";
                return false;
            }
            snprintf(out->textures[out->texture_count++], WGR_SHADER_NAME_MAX, "%s", a);
        } else if (sscanf(line, "program %63s %63s", a, b) == 2) {
            const int which = strcmp(a, "static") == 0          ? WGR_SHADER_PROGRAM_STATIC
                              : strcmp(a, "skinned") == 0       ? WGR_SHADER_PROGRAM_SKINNED
                              : strcmp(a, "sprite") == 0        ? WGR_SHADER_PROGRAM_SPRITE
                              : strcmp(a, "sprite_pulled") == 0 ? WGR_SHADER_PROGRAM_SPRITE_PULLED
                              : strcmp(a, "screen") == 0        ? WGR_SHADER_PROGRAM_SCREEN
                                                                : -1;
    ...
    for (int i = 0; i < WGR_SHADER_PROGRAM_COUNT; i++) {
...
static void free_programs(program_desc_t programs[WGR_SHADER_PROGRAM_COUNT])
{
    for (int i = 0; i < WGR_SHADER_PROGRAM_COUNT; i++) {
...
static void destroy_gpu(wgr_shader_t *shader)
{
    for (int s = 0; s < 2; s++) {
        for (int b = 0; b < 2; b++) {
            for (int d = 0; d < 2; d++) {
    ...
    for (int p = 0; p < WGR_SHADER_SPRITE_PIPELINES; p++) {
    ...
    for (int s = 0; s < WGR_SHADER_PROGRAM_COUNT; s++) {
...
static wgr_loader_step_t finish_shader(void *data, const char *path, wgr_handle_t *resource)
{
    ...
    *resource = 0;
    ...
    programs = calloc(WGR_SHADER_PROGRAM_COUNT, sizeof(program_desc_t));
    memset(&shader, 0, sizeof(shader));
    ok = programs != NULL && parse(file->bytes, file->size, slang, &shader, programs, &error);
    for (int i = 0; ok && i < WGR_SHADER_PROGRAM_COUNT; i++) {
        program_desc_t *p = &programs[i];
        wgr_shader_program_t *program = &shader.programs[i];
        ...
        static const char *labels[WGR_SHADER_PROGRAM_COUNT] = {"wgr-custom-static", "wgr-custom-skinned",
        ...
        for (int t = 0; t < WGR_SHADER_MAX_TEXTURES; t++) {
        ...
        for (int t = 0; t < shader.texture_count; t++) {
            for (int v = 0; v < WGR_SHADER_MAX_TEXTURES; v++) {
    ...
    shader.ref_count = 1;
    const wgr_handle_t handle = wgr_handle_pool_alloc(&wgr_shader_pool);
    if (handle == 0) {
        log_error("shader: pool full (%u)", (unsigned)wgr_shader_pool.max - 1u);
        destroy_gpu(&shader);
        return WGR_LOADER_FAILED;
    }
    wgr_handle_pool_resolve(&wgr_shader_pool, handle, &index);
...
static wgr_handle_t find_shader(const char *path)
{
    ...
    for (uint16_t i = 1; i < wgr_shader_pool.capacity; i++) {
        if (wgr_shader_pool.occupied[i] && wgr_shaders[i].has_path && strcmp(wgr_shaders[i].path, path) == 0) {
            ...
            return wgr_handle_pool_handle_from_index(&wgr_shader_pool, i);
        }
    }
    return 0;
}

static const wgr_loader_t wgr_shader_loader = {
    .name = "shader",
...
{
    return wgr_loader_create(&wgr_shader_loader, path);
}

void wgr_shader_retain(wgr_handle_t shader)
{
    wgr_shader_t *shader_ptr = resolve(shader);
...
void wgr_shader_release(wgr_handle_t shader)
{
    ...
    if (shader_ptr->ref_count == 0) {
        ...
        wgr_handle_pool_free(&wgr_shader_pool, shader);
    }
}

void wgr_shader_init(void)
{
    if (!wgr_handle_pool_init(&wgr_shader_pool, WGR_HANDLE_KIND_SHADER, "shader", (void **)&wgr_shaders,
                             sizeof(wgr_shader_t), SHADERS_INITIAL, WGR_HANDLE_POOL_MAX_SLOTS)) {
        log_error("shader: out of memory");
    }
    wgr_asset_register_loader(".wgrshader", &wgr_shader_loader);
    wgr_shader_hooks = (wgr_shader_hooks_t){
        .get = wgr_shader_get,
        .retain = wgr_shader_retain,
        .release = wgr_shader_release,
        .find_param = wgr_shader_find_param,
        .find_texture = wgr_shader_find_texture,
        .fallbacks = fallbacks,
    };
}
...
void wgr_shader_deinit(void)
{
    ...
    wgr_handle_pool_destroy(&wgr_shader_pool);
    if (wgr_shader_fallback.linear.id != SG_INVALID_ID) {
    ...
    wgr_shader_hooks = (wgr_shader_hooks_t){0};
}
...
static wgr_module_t wgr_shader_module = {.name = "shader", .order = 25, .init = wgr_shader_init,
                                       .deinit = wgr_shader_deinit};
WGR_MODULE(wgr_shader_module)
