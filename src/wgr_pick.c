...
static inline vec3_t mul_direction(wgri_mat4_t m, vec3_t d)
{
    return (vec3_t){
                                 ...
                                 float screen_w, float screen_h)
{
    ...
    inv_vp = wgri_mat4_inverse(wgri_mat4_mul(proj, view));

    ndc_x = 2.0f * mouse_x / screen_w - 1.0f;
    ndc_y = 1.0f - 2.0f * mouse_y / screen_h;
    near_p = wgri_mat4_mul_point(inv_vp, (vec3_t){ndc_x, ndc_y, -1.0f});
    far_p = wgri_mat4_mul_point(inv_vp, (vec3_t){ndc_x, ndc_y, 1.0f});
    ray.origin = near_p;
    ray.dir = wgri_v3_norm(wgri_v3_sub(far_p, near_p));
    return ray;
}

wgri_ray_t wgri_pick_ray_to_local(wgri_mat4_t model, wgri_ray_t world)
{
    wgri_mat4_t inv = wgri_mat4_inverse(model);
    wgri_ray_t local;
    float len;

    local.origin = wgri_mat4_mul_point(inv, world.origin);
...
{
    vec3_t oc = wgri_v3_sub(ray.origin, center);
    ...
    out->normal = wgri_v3_norm(wgri_v3_sub(out->point, center));
...
bool wgri_pick_ray_triangle(wgri_ray_t ray, vec3_t v0, vec3_t v1, vec3_t v2, wgri_ray_hit_t *out)
{
    const float eps = 1e-7f;
    vec3_t e1 = wgri_v3_sub(v1, v0);
    vec3_t e2 = wgri_v3_sub(v2, v0);
    vec3_t p = wgri_v3_cross(ray.dir, e2);
    ...
    inv = 1.0f / det;
    tvec = wgri_v3_sub(ray.origin, v0);
    u = v3_dot(tvec, p) * inv;
    if (u < 0.0f || u > 1.0f) {
        return false;
    }
    q = wgri_v3_cross(tvec, e1);
    ...
    n = wgri_v3_norm(wgri_v3_cross(e1, e2));
                               ...
                               wgri_mat4_t model, wgr_pick_result_t *out)
{
    ...
    out->normal_local = wgri_v3_norm(local_hit->normal);
    out->point_world = wgri_mat4_mul_point(model, local_hit->point);
    out->normal_world = wgri_v3_norm(mul_direction(model, local_hit->normal));
                               ...
                               wgri_mat4_t model, wgr_pick_result_t *out)
{
    ...
    inv = wgri_mat4_inverse(model);
    out->hit = true;
    out->point_world = world_hit->point;
    out->normal_world = wgri_v3_norm(world_hit->normal);
    out->point_local = wgri_mat4_mul_point(inv, world_hit->point);
    out->normal_local = wgri_v3_norm(mul_direction(inv, world_hit->normal));
    out->distance = v3_dist(world_hit->point, world_ray.origin);
}

void wgri_pick_world_aabb(vec3_t lmin, vec3_t lmax, wgri_mat4_t model, vec3_t *wmin, vec3_t *wmax)
{
    ...
    for (int i = 0; i < 8; i++) {
        vec3_t c = {(i & 1) ? lmax.x : lmin.x,
                    (i & 2) ? lmax.y : lmin.y,
                    (i & 4) ? lmax.z : lmin.z};
        vec3_t w = wgri_mat4_mul_point(model, c);
...
bool wgri_pick_ray_world_aabb(wgri_ray_t ray, vec3_t lmin, vec3_t lmax, wgri_mat4_t model, float *t_out)
{
    vec3_t wmin, wmax;
    wgri_ray_hit_t hit = {0};

    wgri_pick_world_aabb(lmin, lmax, model, &wmin, &wmax);
    if (!wgri_pick_ray_aabb(ray, wmin, wmax, &hit)) {
        return false;
    }
    if (t_out != NULL) {
        *t_out = hit.t;
