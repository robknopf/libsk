...
static inline vec3_t mul_direction(sk_mat4_t m, vec3_t d)
{
    return (vec3_t){
                                 ...
                                 float screen_w, float screen_h)
{
    ...
    near_p = sk_mat4_mul_point(inv_vp, (vec3_t){ndc_x, ndc_y, -1.0f});
    far_p = sk_mat4_mul_point(inv_vp, (vec3_t){ndc_x, ndc_y, 1.0f});
    ray.origin = near_p;
    ray.dir = sk_v3_norm(sk_v3_sub(far_p, near_p));
    return ray;
}

sk_ray_t sk_pick_ray_to_local(sk_mat4_t model, sk_ray_t world)
{
    sk_mat4_t inv = sk_mat4_inverse(model);
    sk_ray_t local;
    float len;

    local.origin = sk_mat4_mul_point(inv, world.origin);
...
{
    vec3_t oc = sk_v3_sub(ray.origin, center);
...
bool sk_pick_ray_triangle(sk_ray_t ray, vec3_t v0, vec3_t v1, vec3_t v2, sk_ray_hit_t *out)
{
    const float eps = 1e-7f;
    vec3_t e1 = sk_v3_sub(v1, v0);
    vec3_t e2 = sk_v3_sub(v2, v0);
    vec3_t p = sk_v3_cross(ray.dir, e2);
    ...
    inv = 1.0f / det;
    tvec = sk_v3_sub(ray.origin, v0);
    u = v3_dot(tvec, p) * inv;
    if (u < 0.0f || u > 1.0f) {
        return false;
    }
    q = sk_v3_cross(tvec, e1);
    ...
    n = sk_v3_norm(sk_v3_cross(e1, e2));
                               ...
                               sk_mat4_t model, sk_pick_result_t *out)
{
    ...
    out->normal_local = sk_v3_norm(local_hit->normal);
    out->point_world = sk_mat4_mul_point(model, local_hit->point);
    out->normal_world = sk_v3_norm(mul_direction(model, local_hit->normal));
                               ...
                               sk_mat4_t model, sk_pick_result_t *out)
{
    ...
    inv = sk_mat4_inverse(model);
    out->hit = true;
    out->point_world = world_hit->point;
    out->normal_world = sk_v3_norm(world_hit->normal);
    out->point_local = sk_mat4_mul_point(inv, world_hit->point);
    out->normal_local = sk_v3_norm(mul_direction(inv, world_hit->normal));
    out->distance = v3_dist(world_hit->point, world_ray.origin);
}

void sk_pick_world_aabb(vec3_t lmin, vec3_t lmax, sk_mat4_t model, vec3_t *wmin, vec3_t *wmax)
{
    ...
    for (int i = 0; i < 8; i++) {
        vec3_t c = {(i & 1) ? lmax.x : lmin.x,
                    (i & 2) ? lmax.y : lmin.y,
                    (i & 4) ? lmax.z : lmin.z};
        vec3_t w = sk_mat4_mul_point(model, c);
...
bool sk_pick_ray_world_aabb(sk_ray_t ray, vec3_t lmin, vec3_t lmax, sk_mat4_t model, float *t_out)
{
    vec3_t wmin, wmax;
    sk_ray_hit_t hit = {0};

    sk_pick_world_aabb(lmin, lmax, model, &wmin, &wmax);
    if (!sk_pick_ray_aabb(ray, wmin, wmax, &hit)) {
        return false;
    }
    if (t_out != NULL) {
        *t_out = hit.t;
