...
static wgri_handle_pool_t wgr_text3d_pool;
...
static bool use_font(wgr_handle_t font)
{
    ...
    const int id = wgri_font_fons_id(wgri_text_resolve_font(font)); /* 0: the default font */
 ...
 * splitting text2d uses (wgri_text_split_lines), since text3d draws its own glyphs. */
typedef struct {
...
static bool layout_block(wgr_handle_t font, const char *text, float size, float max_width, text3d_block_t *out)
{
    ...
    out->count = wgri_text_split_lines(text, -1, max_width > 0.0f ? max_width * to_raster : 0.0f, out->starts, out->ends,
                                     MAX_TEXT3D_LINES);
    if (out->count == 0) {
        return false;
    }
...
static void quad_corners(vec3_t center, vec3_t right, vec3_t up, vec2_t extent, vec3_t *tl, vec3_t *tr, vec3_t *br,
                         vec3_t *bl)
{
    const vec3_t r = wgri_v3_scale(right, extent.x * 0.5f), u = wgri_v3_scale(up, extent.y * 0.5f);
                      ...
                      wgr_text_align_t align_y, vec3_t position, vec3_t right, vec3_t up, wgr_color_t color)
{
    ...
    if (!layout_block(font, text, size, max_width, &block) ||
        !wgri_fontstash_render_state(fons, &atlas, &sampler, &shader)) {
        return;
    }
...
static void draw_handle(wgr_handle_t handle)
{
    ...
    if (text_ptr == NULL || !text_ptr->visible || text_ptr->text == NULL || !wgri_camera3d_get_active_data(&cam)) {
        return;
    }
    wgri_sprite3d_facing_basis(text_ptr->facing, text_ptr->rotation, &cam, &right, &up);
                               ...
                               int max_items)
{
    ...
    out[0] = (wgri_transparent_item_t){
        .handle = handle,
        .part = 0,
        .depth = wgri_scene_view_depth(cam, text_ptr->position),
    };
    return 1;
}
...
static bool text_bounds(wgr_handle_t handle, vec3_t *lmin, vec3_t *lmax, wgri_mat4_t *model)
{
    const wgr_text3d_t *text_ptr = resolve(handle);
    ...
    *lmin = (vec3_t){-r, -r, -r};
    *lmax = (vec3_t){r, r, r};
    *model = wgri_mat4_translate(text_ptr->position.x, text_ptr->position.y, text_ptr->position.z);
    return true;
}
...
static bool text_pick(wgr_handle_t handle, vec3_t origin, vec3_t dir, wgr_pick_result_t *out)
{
    ...
    quad_corners(block_center(text_ptr->position, right, up, extent, text_ptr->align_x, text_ptr->align_y), right, up,
                 extent, &tl, &tr, &br, &bl);
    const bool got0 = wgri_pick_ray_triangle(ray, tl, tr, br, &h0);
    const bool got1 = wgri_pick_ray_triangle(ray, tl, br, bl, &h1);
    const wgri_ray_hit_t *best = got0 && (!got1 || h0.t <= h1.t) ? &h0 : (got1 ? &h1 : NULL);
    if (best != NULL) {
        wgri_pick_result_from_world(best, ray,
                                  wgri_mat4_translate(text_ptr->position.x, text_ptr->position.y, text_ptr->position.z),
                                  out);
    } else {
...
void wgri_text3d_init(void)
{
    ...
    wgri_scene_register_pick(WGR_HANDLE_KIND_TEXT3D, text_pick);
    wgri_scene_register_enabled(WGR_HANDLE_KIND_TEXT3D, wgr_text3d_is_enabled);
}
...
void wgri_text3d_deinit(void)
{
    ...
    wgri_handle_pool_destroy(&wgr_text3d_pool);
}

/* An optional subsystem: part of the runtime when a program uses it (internal/wgri_module.h). */
static wgri_module_t wgr_text3d_module = {.name = "text3d", .order = 81, .init = wgri_text3d_init, .deinit = wgri_text3d_deinit};
WGRI_MODULE(wgr_text3d_module)
