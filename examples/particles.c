...
static sk_handle_t g_confetti[CONFETTI_COLORS];
...
static void make_smoke(sk_handle_t texture)
{
    ...
    sk_emitter3d_set_size(g_smoke, 0.8f, 3.2f, 0.3f);
    sk_emitter3d_set_spin(g_smoke, -0.8f, 0.8f);
    sk_emitter3d_set_color(g_smoke, sk_color_rgba(120, 120, 130, 150), sk_color_rgba(70, 70, 80, 0));
...
{
    const sk_color_t colors[CONFETTI_COLORS] = {
        SK_COLOR_RED, SK_COLOR_GOLD, SK_COLOR_LIME, SK_COLOR_SKYBLUE, SK_COLOR_VIOLET,
    };
    for (int i = 0; i < CONFETTI_COLORS; i++) {
        sk_handle_t confetti = sk_emitter2d_create(texture);
        g_confetti[i] = confetti;
        sk_emitter2d_set_source(confetti, 24.0f, 24.0f, 16.0f, 16.0f); /* the dot's solid middle: squares */
        sk_emitter2d_set_max(confetti, 1024);
        sk_emitter2d_set_life(confetti, 1.2f, 2.2f);
        sk_emitter2d_set_velocity(confetti, 0.0f, -420.0f, 1.3f, 0.7f);
        sk_emitter2d_set_gravity(confetti, 0.0f, 700.0f);
        sk_emitter2d_set_size(confetti, 12.0f, 6.0f, 0.5f);
        sk_emitter2d_set_spin(confetti, -8.0f, 8.0f);
        sk_emitter2d_set_color(confetti, colors[i], sk_color_with_alpha(colors[i], 0));
        sk_emitter2d_set_alpha_mode(confetti, SK_ALPHA_BLEND, 0.0f);
        sk_scene_add(g_scene, confetti, 1);
    }
}
...
{
    for (int i = 0; i < CONFETTI_COLORS; i++) {
        sk_emitter2d_set_position(g_confetti[i], x, y);
        sk_emitter2d_burst(g_confetti[i], 60);
    }
}
...
static void frame(float dt, float tick_fraction, void *user_data)
{
    ...
    if (mouse.left == SK_BUTTON_PRESSED && g_confetti[0] != 0) {
    ...
    int confetti = 0;
    for (int i = 0; i < CONFETTI_COLORS; i++) {
        confetti += sk_emitter2d_get_count(g_confetti[i]);
    }
    ...
    snprintf(line, sizeof(line), "fountain %d   sparks %d   smoke %d   confetti %d",
             ...
             sk_emitter3d_get_count(g_smoke), confetti);
