 ...
 *     wgr_sprite3d_set_source
 *   - trees and flags are 16x32 in the sheet, drawn 1x2 world units with
 *     wgr_sprite3d_set_extent, and stand on their cell because their pivot is the
 *     bottom edge (wgr_sprite3d_set_pivot(0.5, 1))
 *   - drag or use the arrow keys to scroll, the wheel to zoom (the camera's
 *     orthographic height)
 *   - the sheet is pixel art (alpha 0 or 1): ground tiles are WGR_ALPHA_OPAQUE, props
 *     WGR_ALPHA_MASK, so nothing needs sorting and each layer draws in one batch
...
static struct {
    ...
    wgr_color_t bg, shade, text, dim, highlight;
    wgr_handle_t ground[WORLD_W * WORLD_H];
    wgr_handle_t props[MAX_PROPS];
...
{
    wgr_camera3d_set_view(g.camera, g.center_x, g.center_y, 10.0f, g.center_x, g.center_y, 0.0f, 0, 1, 0);
    wgr_camera3d_set_ortho_height(g.camera, g.zoom);
}
...
static wgr_handle_t add_sprite(const float cell[4], float x, float y, float z, float width, float height,
                              float pivot_y, int layer)
{
    const wgr_handle_t sprite = wgr_sprite3d_create(g.texture);
    wgr_sprite3d_set_facing(sprite, WGR_SPRITE3D_FACING_FREE);
    wgr_sprite3d_set_source(sprite, cell[0], cell[1], cell[2], cell[3]);
    wgr_sprite3d_set_extent(sprite, width, height);
    wgr_sprite3d_set_pivot(sprite, 0.5f, pivot_y);
    wgr_sprite3d_set_transform(sprite, x, y, z, 0, 0, 0, 1, 1, 1);
    ...
    wgr_sprite3d_set_alpha_mode(sprite, layer == LAYER_GROUND ? WGR_ALPHA_OPAQUE : WGR_ALPHA_MASK, 0.5f);
    wgr_scene_add(g.scene, sprite, layer);
    return sprite;
}
...
static void add_prop(const float cell[4], float x, float y, float width, float height, bool coin)
{
    ...
    if (coin) {
        wgr_sprite3d_set_pick_alpha_test(g.props[g.prop_count], true, 0.5f);
    } else {
        wgr_sprite3d_set_pickable(g.props[g.prop_count], false);
    }
    g.prop_count++;
}
...
static void on_tiles(const char *path, void *user)
{
    ...
    for (int y = 0; y < WORLD_H; y++) {
        for (int x = 0; x < WORLD_W; x++) {
            ...
            wgr_sprite3d_set_pickable(g.ground[y * WORLD_W + x], false);
        }
    }
...
static void init(void *user_data)
{
    ...
    g.shade = wgr_color_rgba(18, 20, 28, 190);
    g.text = wgr_color_rgba(235, 238, 245, 255);
    g.dim = wgr_color_rgba(140, 146, 158, 255);
    g.highlight = wgr_color_rgba(255, 230, 140, 255);

    g.center_x = WORLD_W * 0.5f;
    g.center_y = WORLD_H * 0.5f;
    g.zoom = 12.0f;
    g.camera = wgr_camera3d_create(WGR_CAMERA3D_ORTHOGRAPHIC);
    place_camera();
...
static void frame(float dt, float tick_fraction, void *user_data)
{
    ...
    if (mouse.buttons[0] == WGR_BUTTON_DOWN) {
    ...
    }
    {
        ...
        if (kb.keys[WGR_KEY_LEFT] >= WGR_BUTTON_PRESSED && kb.keys[WGR_KEY_LEFT] != WGR_BUTTON_RELEASED) g.center_x -= speed;
        if (kb.keys[WGR_KEY_RIGHT] >= WGR_BUTTON_PRESSED && kb.keys[WGR_KEY_RIGHT] != WGR_BUTTON_RELEASED) g.center_x += speed;
        if (kb.keys[WGR_KEY_DOWN] >= WGR_BUTTON_PRESSED && kb.keys[WGR_KEY_DOWN] != WGR_BUTTON_RELEASED) g.center_y -= speed;
        if (kb.keys[WGR_KEY_UP] >= WGR_BUTTON_PRESSED && kb.keys[WGR_KEY_UP] != WGR_BUTTON_RELEASED) g.center_y += speed;
    }
    ...
    for (int i = 0; i < g.prop_count; i++) {
        const wgr_button_state_t hover = wgr_scene_get_hover(g.scene, g.props[i]);
        if (!g.is_coin[i]) continue;
        wgr_sprite3d_set_tint(g.props[i],
                             hover == WGR_BUTTON_PRESSED || hover == WGR_BUTTON_DOWN ? g.highlight : WGR_COLOR_WHITE);
        if (wgr_scene_is_clicked(g.scene, g.props[i])) {
            wgr_sprite3d_set_visible(g.props[i], false);
            wgr_sprite3d_set_pickable(g.props[i], false);
            g.collected++;
        }
    }
