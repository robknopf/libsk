...
void main() {
    vec4 color = wgr_sprite_color();
    /* the outline: a see-through pixel next to an opaque one */
    vec2 step = outline_width / vec2(textureSize(sampler2D(wgr_sprite_tex, wgr_sprite_smp), 0));
    float near = 0.0;
    for (int i = 0; i < 8; i++) {
        float a = 6.2831853 * float(i) / 8.0;
        near = max(near, texture(sampler2D(wgr_sprite_tex, wgr_sprite_smp), wgr_uv0 + vec2(cos(a), sin(a)) * step).a);
    }
    float pulse = flash * (0.5 + 0.5 * sin(wgr_time() * pulse_speed));
