...
{
    echo "# Vendored from libwgrender's fork of nicbarker/clay (zlib license, LICENSE.md): a C99 UI"
    echo "# layout library (flexbox-like; emits render commands and draws nothing itself), with"
    echo "# fixes libwgrender needs on top of upstream. Used only by examples/clay.c, which draws it"
    echo "# through libwgrender's public API; libwgrender itself doesn't include or link Clay"
