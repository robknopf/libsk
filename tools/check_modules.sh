#!/usr/bin/env bash
# Enforce the optional-subsystem boundary (src/internal/sk_module.h): the core must
...
# Optional subsystems are the objects that register a module (SK_MODULE); the core
# reaches them only through the module list and the hooks in sk_render.h / sk_scene.h.
#
...
for line in sys.stdin:  # "lib.a:obj.o:ADDRESS TYPE NAME", or "lib.a:obj.o: U NAME"
    ...
    if name.startswith("sk_register_") and name.endswith("_module"):
        optional.add(obj)
bad = []
...
if bad:
    print("FAIL: core code calls optional subsystems by name (go through sk_module.h and the hooks):")
    print("\n".join(bad))
    sys.exit(1)
